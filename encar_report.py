...
def _render_report_template(data_ru: dict, base_dir: Path | None = None, use_file_url: bool = False, diag: dict | None = None) -> str:
    ...
    for row in summary_list:
        ...
        # Для полей «Да/Нет» показываем только фактическое значение, а не «Нет Да»
        if val_actual:
            if val in OPTION_PAIRS or (val_actual in val and len(val.split()) >= 2):
                ...
                row["value"] = val_actual
                row["value_actual"] = ""
        elif val in OPTION_PAIRS and status_str in ("Нет", "Да"):
            # Парсер не нашёл .txt_state.on — используем status как фактический выбор (на Encar он совпадает с выбором)
            row["value"] = status_str
            row["value_actual"] = ""
